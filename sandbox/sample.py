...
out_dir = "logs/gpt2-s-1696101844.556695"  # ignored if init_from is not 'resume'
start = input(
    "Enter prompt: "
...
all_data = np.load(f"./datasets/arxiv_abstracts/train_val_split.npz", allow_pickle=True)
with open("./datasets/arxiv_abstracts/embeddings.json", "r") as file:
...
def decode(emb):
    doc = []
    for i in emb:
        doc.append(inverted_embeddings_index[i])
    return "".join(doc)
