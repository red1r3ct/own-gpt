...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.causal_mask = self.causal_mask.eq(float('-inf')).cast(torch.bool)
        self.causal_mask = self.causal_mask.to(config.device)

    def forward(self, x, key_padding_mask=None):
        x, mask = self.attn(x, x, x, key_padding_mask=key_padding_mask, is_causal=True, attn_mask=self.causal_mask)
        return x
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if self.config.pad_token_idx is not None:
            attn_mask = idx.eq(self.config.pad_token_idx).cast(torch.bool)
    ...
    def estimate_mfu(self, fwdbwd_per_iter, dt):
        ...
        flops_promised = 2.6e12  # A100 GPU bfloat16 peak flops is 312 TFLOPS
